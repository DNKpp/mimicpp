...
MIMICPP_DETAIL_MODULE_EXPORT namespace mimicpp
{
    ...
    /**
     * \defgroup TYPE_TRAITS_SIGNATURE_ADD_LVALUE_REF_QUALIFIER signature_add_lvalue_ref_qualifier
     * \ingroup TYPE_TRAITS
     * \brief Adds the lvalue-ref qualifier to a signature (if not already present).
     * \note This will yield a compile-error when the signature already has the rvalue-ref qualifier.
     *
     *\{
     */
    ...
    {
        template <typename Signature, bool isNoexcept>
        struct signature_add_lvalue_ref_qualifier_impl;

        template <typename Return, typename... Params, bool isNoexcept>
        struct signature_add_lvalue_ref_qualifier_impl<Return(Params...), isNoexcept>
            : public std::true_type
        {
            using type = Return(Params...) & noexcept(isNoexcept);
        };
        ...
        template <typename Return, typename... Params, bool isNoexcept>
        struct signature_add_lvalue_ref_qualifier_impl<Return(Params...) const, isNoexcept>
            : public std::true_type
        {
            using type = Return(Params...) const& noexcept(isNoexcept);
        };
        ...
        // Todo: When msvc some day supports deducing noexcept, we can further simplify this.
        template <typename Signature, typename Trait = signature_remove_noexcept<Signature>>
        struct signature_add_lvalue_ref_qualifier_
            : public signature_add_lvalue_ref_qualifier_impl<typename Trait::type, Trait::value>
        {
        };

        template <typename Signature>
        using signature_add_lvalue_ref_qualifier = signature_add_lvalue_ref_qualifier_<Signature>;
    }
