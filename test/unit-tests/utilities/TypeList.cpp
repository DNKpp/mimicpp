    ...
    "[detail][utility]")
{
    ...
    SECTION("When tuple is empty")
    {
        ...
        SECTION("And n == 0")
        {
            const std::tuple result = util::detail::expand_tuple<int, 0>(std::move(input));
            STATIC_REQUIRE(std::same_as<const std::tuple<>, decltype(result)>);
        }

        SECTION("And n == 1")
        {
            const std::tuple result = util::detail::expand_tuple<int, 1>(std::move(input));
        ...
        SECTION("And n == 2")
        {
            const std::tuple result = util::detail::expand_tuple<int, 2>(std::move(input));
    ...
    SECTION("When tuple has size 1")
    {
        ...
        SECTION("And n == 3")
        {
            const std::tuple result = util::detail::expand_tuple<int, 3>(std::move(input));
    ...
    SECTION("When tuple has size 2")
    {
        ...
        SECTION("And n == 4")
        {
            const std::tuple result = util::detail::expand_tuple<int, 4>(std::move(input));
    ...
    SECTION("It keeps the existing elements as they are.")
    {
        ...
        {
            const std::tuple result = util::detail::expand_tuple<int, 1>(std::tuple<const std::string>(arg));
        ...
        {
            const std::tuple result = util::detail::expand_tuple<int, 1>(std::tuple<std::string&>(arg));
        ...
        {
            const std::tuple result = util::detail::expand_tuple<int, 1>(std::tuple<std::string&&>(std::move(arg)));
            STATIC_REQUIRE(std::same_as<const std::tuple<std::string&&>, decltype(result)>);
            REQUIRE(&arg == &std::get<0>(result));
        }
        ...
        {
            const std::tuple result = util::detail::expand_tuple<int, 1>(std::tuple<const std::string&&>(std::move(arg)));
            STATIC_REQUIRE(std::same_as<const std::tuple<const std::string&&>, decltype(result)>);
            REQUIRE(&arg == &std::get<0>(result));
        }
    }
