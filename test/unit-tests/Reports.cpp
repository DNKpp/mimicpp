...
TEST_CASE(
    ...
    "[reporting]")
{
    TypeReport const report = TypeReport::make<std::string>();

    SECTION("Two reports compare equal, if they both are constructed for the same type.")
    {
        TypeReport const other = TypeReport::make<std::string>();
    ...
    {
        TypeReport const constOther = TypeReport::make<std::string const>();
        REQUIRE_FALSE(report == constOther);
        REQUIRE(report != constOther);

        TypeReport const refOther = TypeReport::make<std::string&>();
    ...
    {
        TypeReport const other = TypeReport::make<int>();
        REQUIRE_FALSE(report == other);
        REQUIRE(report != other);
    }
}
