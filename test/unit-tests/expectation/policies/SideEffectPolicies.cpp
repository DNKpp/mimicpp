...
TEST_CASE(
    "Invalid configurations of expectation::policies::SideEffectAction do not satisfy expectation_policy_for concept.",
    ...
    SECTION("When action is not applicable.")
    {
        STATIC_REQUIRE(
            !expectation::expectation_policy_for<
                expectation::policies::SideEffectAction<ActionT>,
                void(int)>);
    }
}

TEST_CASE(
    "expectation::policies::SideEffectAction invokes the specified function on consume.",
    "[expectation][expectation::policy]")
{
    using trompeloeil::_;
    ...
    InvocableMock<void, call::Info<void> const&> action{};
...
TEST_CASE(
    "then::apply_arg creates expectation::policies::SideEffectAction.",
    "[expectation][expectation::factories]")
{
    ...
    call::Info<void, int&, double&, std::string&> const info{
    ...
    SECTION("Index 0.")
    {
        ...
        expectation::policies::SideEffectAction policy = then::apply_arg<0>(std::ref(action));
        REQUIRE_CALL(action, Invoke(_))
            .LR_WITH(&_1 == &param0);
        REQUIRE_NOTHROW(policy.consume(info));
    }
    ...
    SECTION("Index 1.")
    {
        InvocableMock<void, double const&> action{};
        expectation::policies::SideEffectAction policy = then::apply_arg<1>(std::ref(action));
        REQUIRE_CALL(action, Invoke(_))
            .LR_WITH(&_1 == &param1);
        REQUIRE_NOTHROW(policy.consume(info));
    }
    ...
    SECTION("Index 2.")
    {
        ...
        expectation::policies::SideEffectAction policy = then::apply_arg<2>(std::ref(action));
...
TEST_CASE(
    "then::apply_args creates expectation::policies::SideEffectAction.",
    "[expectation][expectation::factories]")
{
    using trompeloeil::_;
    ...
    SECTION("Indices 0, 1, 2.")
    {
        ...
        expectation::policies::SideEffectAction policy = then::apply_args<0, 1, 2>(std::ref(action));
    ...
    SECTION("Indices 2, 1, 0")
    {
        ...
        expectation::policies::SideEffectAction policy = then::apply_args<2, 1, 0>(std::ref(action));
        REQUIRE_CALL(action, Invoke(_, _, _))
            .LR_WITH(&_1 == &param2)
            .LR_WITH(&_2 == &param1)
            .LR_WITH(&_3 == &param0);
    ...
    SECTION("Arbitrarily mixed.")
    {
        ...
        expectation::policies::SideEffectAction policy = then::apply_args<1, 0, 2, 2, 0>(std::ref(action));
...
TEST_CASE(
    "then::apply_all creates expectation::policies::SideEffectAction.",
    "[expectation][expectation::factories]")
{
    using trompeloeil::_;
    ...
    SECTION("When signature has zero params.")
    {
        ...
        expectation::policies::SideEffectAction policy = then::apply_all(std::ref(action));
    ...
    SECTION("When signature has multiple params.")
    {
        int param0{1337};
        double param1{4.2};
        call::Info<void, int&, double&> const info{
            .args = {param0, param1}
        };
...
TEST_CASE(
    "then::invoke creates expectation::policies::SideEffectAction.",
    "[expectation][expectation::factories]")
{
    int param0{1337};
    double param1{4.2};
    ...
    InvocableMock<void> action{};
    expectation::policies::SideEffectAction policy = then::invoke(std::ref(action));
    REQUIRE_CALL(action, Invoke());
    REQUIRE_NOTHROW(policy.consume(info));
}
