...
)
{
	...
	int value{42};
	SCOPED_EXP mock.expect_call(42)
				| then::apply_param<0>([](auto& v) { v = 1337; });
...
)
{
	...
	SCOPED_EXP mock.expect_call(42)
				| then::apply_param<0>([](auto& v) { v = 1337; })
				| then::apply_param<0>([](auto& v) { v = (v == 1337 ? -1337 : -42); });
...
)
{
	...
	SCOPED_EXP mock.expect_call(0, 1, 2)
				| then::apply_params<1, 2, 0>(	// note the index order:
...
)
{
	...
	SCOPED_EXP mock.expect_call(0, 42)
				| then::apply_params<1, 1, 0>(	// note the indices: second param is applied twice
				[](int lhs, int rhs, int& out) { out = lhs + rhs; });
